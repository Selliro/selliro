...
$(function () {
    ...
    $(function () {
        ...
        // Initial load: always start in dark mode (do not restore light from localStorage)
        // User can still toggle to light; that choice persists until they toggle back.

        var themeBtn = document.querySelector(".theme-icon");
        if (themeBtn) {
        ...
        // Ensure logos match current mode: dark mode = logo-light, light mode = logo-dark
        if (isLight()) {
   ...
   ============================================================================= */
    ...
    function deleteText(element, speed, callback) {
        // Store the original text to ensure we delete correctly
        let text = element.textContent;
        let i = text.length;
        
        function deleteChar() {
            if (i > 0) {
                // Remove one character at a time - clearly visible
    ...
    function startTypingAnimation() {
        ...
        const words = ['design.', 'develop.', 'solutions.'];
        let currentWordIndex = 0;
        const typingSpeed = 60; // milliseconds per character (smooth typing)
        const deletingSpeed = 50; // milliseconds per character (visible one-by-one deletion)
        const pauseTime = 2000; // pause time after typing each word
        const pauseBetweenWords = 400; // pause between deleting and typing next word

        function cycleWords() {
            const currentWord = words[currentWordIndex];
            
            // Type the current word
            typeWriter(typingElement, currentWord, typingSpeed, function() {
                // After typing, wait, then delete one by one
                setTimeout(function() {
                    deleteText(typingElement, deletingSpeed, function() {
                        // Move to next word
                        currentWordIndex = (currentWordIndex + 1) % words.length;
                        // Start typing next word after a short pause
                        setTimeout(cycleWords, pauseBetweenWords);
                    });
                }, pauseTime);
            });
        }
...
/* =============================================================================
-----------------------------  Hero demo: GSAP scroll-speed  ---------------------
============================================================================= */

(function initHeroDemoScrollSpeed() {
    var wrapper = document.querySelector('.nm-hero-demo');
    if (!wrapper || typeof gsap === 'undefined') return;

    var cols = wrapper.querySelectorAll('.nm-hero-demo-col');
    ...
    // Disable CSS animation so GSAP controls transform
    col1.style.animation = 'none';
    col2.style.animation = 'none';

    // Column 1: 1% -> -60% (same as heroDemoSlide)
    tl1 = gsap.fromTo(col1, { y: '1%' }, {
        y: '-60%',
    ...
    // Column 2: -60% -> 0% (same as heroDemoSlide2)
    tl2 = gsap.fromTo(col2, { y: '-60%' }, {
        y: '0%',
        duration: baseDuration,
        ease: 'none',
        repeat: -1
    ...
    function onScroll() {
        gsap.to([tl1, tl2], { timeScale: 3.5, duration: 0.35, overwrite: true });
        if (scrollTimeout) clearTimeout(scrollTimeout);
        scrollTimeout = setTimeout(function () {
            gsap.to([tl1, tl2], { timeScale: 1, duration: 0.8, overwrite: true });
        }, 280);
    }
...
function initWhatsAppFloat() {
    ...
    if (!btn) return;
    var msg = "Hi, I'm interested in this page: " + window.location.href;
    btn.href = "https://wa.me/7411501872?text=" + encodeURIComponent(msg);

    var shown = false;
    function tryShow() {
        if (shown) return;
        shown = true;
        btn.classList.remove('whatsapp-float--hidden');
    }
    ...
    if (typeof ScrollTrigger !== 'undefined') {
        ScrollTrigger.addEventListener('scroll', checkScroll);
        ScrollTrigger.create({ start: '200px top', onEnter: tryShow });
    }
